...
namespace MerchantsGuideToGalaxy.Core.RomanHelp
{
    public static class RomanToDecimalConverter
    {
        public static int Convert(string romanNumber)
        {
            if (!IsValid(romanNumber))
                throw new InvalidRomanNumberException(ErrorMessages.CommandNotSupported);
        ...
        private static int GetSymbolValue(char symbol)
        {
            switch (symbol)
            {
                ...
                case 'M':
                    return 1000;
                default:
                    throw new InvalidRomanSymbolException(ErrorMessages.CommandNotSupported);
            }
        }
    }
}
